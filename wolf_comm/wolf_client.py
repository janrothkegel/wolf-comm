...
class WolfClient:
    ...
    async def __init__(
        self, username: str, password: str, expert_p=None, region=None, client=None, client_lambda=None
    ):
    ...
    def _map_parameter(parameter: dict, parent: str) -> Parameter:
        ...
        if UNIT in parameter:
            ...
            if unit in unit_parameter_classes:
                return unit_parameter_classes[unit](
                    value_id, name, parent, parameter_id, bundle_id, readonly
                )

        if LIST_ITEMS in parameter:
            items = [
                ListItem(list_item[VALUE], list_item[DISPLAY_TEXT])
                for list_item in parameter[LIST_ITEMS]
            ]
            return ListItemParameter(
                value_id, name, parent, items, parameter_id, bundle_id, readonly
            )
